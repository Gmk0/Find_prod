  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(freelance.value.subCategorie, (subCategory, index) => {
        ...
        if (index === 3 && ((_a = props.freelance.data.subCategorie) == null ? void 0 : _a.length) > 4) {
          _push(`<span class="ml-2 text-sm text-gray-500"> (+${ssrInterpolate(props.freelance.data.subCategorie.length - 4)} de plus) </span>`);
        } else {
          _push(`<!---->`);
        }
        _push(`<!--]-->`);
