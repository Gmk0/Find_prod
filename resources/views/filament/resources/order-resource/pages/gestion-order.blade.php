...
<x-filament::card>
    ...
    <div class="px-6 py-4 border-t border-gray-200">
        ...
        <div class="flex items-center">
            @if (!empty($record->service->freelance->user->profile_photo_path))
            <img class="w-12 h-12 rounded-full" src="{{Storage::disk('local')->url($record->service->freelance->user->profile_photo_path) }}"
                alt="">
            @else
            <img class="w-12 h-12 rounded-full" src="{{ $record->service->freelance->user->profile_photo_url }}" alt="">
            @endif
            <div class="ml-4">
                <p class="text-sm text-gray-600 dark:text-gray-300">{{$record->service->freelance->user->name}}</p>
                <p class="text-sm text-gray-600 dark:text-gray-300">
                    {{$record->service->freelance->user->email}}</p>
            </div>
        </div>
    </div>
